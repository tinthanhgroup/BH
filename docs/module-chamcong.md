...
- **"👤 Nhân sự đang làm nhưng chưa có dữ liệu chấm công"** — có trong Nhân sự (đang làm) nhưng không thấy trong `chamcong.json`. Từ 09/2026 có thêm cột **Ghi chú**: nếu khớp `KHONG_CAN_CHAMCONG` hoặc `NHANVIEN_KHONG_CAN_CHAMCONG` (2 hằng số khai báo đầu `<script>` trong `chamcong.html`) thì ghi rõ lý do và dòng hiển thị mờ đi — người này **vẫn nằm trong danh sách** (không bị ẩn/loại), chỉ khác là không tính vào số đếm cảnh báo trên tiêu đề panel (biến `canChuY`). Đây là quyết định của quản lý (nhóm này không bắt buộc chấm công máy), không phải lỗi thiếu dữ liệu.
  - **Địa điểm làm việc = "Phan Rang"** luôn được ưu tiên xét đầu tiên trong `ghiChuKhongCanChamCong()`: mặc định không bắt buộc chấm công, không cần xét thêm chức vụ/phòng/tổ gì khác (dù trên sổ sách vẫn thuộc chi nhánh Nha Trang).
  - `KHONG_CAN_CHAMCONG`: theo Chi nhánh, xét **Chức vụ** và **Phòng** (Phòng dùng riêng cho khối Ban Giám đốc vì chức danh trong đó không cố định). Hiện tại: Nha Trang (chức vụ TVBH/Giám đốc bán hàng/Giám đốc dịch vụ; phòng "Ban Giám đốc NT"), Phú Yên (chức vụ Kế toán trưởng/Giám đốc bán hàng/Giám đốc dịch vụ; phòng "Ban Giám đốc PY").
  - `NHANVIEN_KHONG_CAN_CHAMCONG`: danh sách tên cá nhân được ưu tiên riêng, không theo chức vụ/phòng/tổ (hiện tại: Phạm Ngọc Phan Huy, Lê Văn Thanh, Nguyễn Thị Thúy Sương — riêng Thúy Sương thực tế làm việc khác với Tổ "Nhóm Hỗ trợ BH" đang ghi trong sheet Nhân sự, xem cần cập nhật lại sheet không).
