...
Bộ lọc: khoảng thời gian (**30 ngày / toàn bộ lịch sử** — bỏ nút "7 ngày" ngày 23/09/2026 theo yêu cầu người dùng; nút bấm kiểu `.filter-btn` giống `crm.html`) + **Chi nhánh dạng nút bấm** (đổi 23/09/2026 từ dropdown, theo yêu cầu người dùng — danh sách nút vẫn suy ra động từ dữ liệu, không hardcode) + chọn chiến dịch (vẫn dropdown, danh sách quá dài để làm nút). Chọn Chi nhánh sẽ **tự lọc lại danh sách chiến dịch** trong dropdown kế bên (hàm `updateChienDichOptions()`) — chỉ hiện chiến dịch thuộc đúng chi nhánh đang chọn, tránh nhầm lẫn khi 2 chi nhánh trùng tên chiến dịch. Đổi Chi nhánh sẽ tự bỏ chọn Chiến dịch cũ.
