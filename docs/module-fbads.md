...
5 panel đánh số:
1. **Xu hướng Chi phí theo ngày** (luôn hiện, **không còn đoạn mô tả `<p class="desc">`** — bỏ 23/09/2026 theo yêu cầu người dùng) — chi phí tất cả chiến dịch mỗi ngày, **xếp chồng theo chi nhánh** (mỗi đoạn màu = 1 chi nhánh, màu cố định theo `CHI_NHANH_COLORS`/`CHI_NHANH_ORDER`: Phú Yên=`--navy`, Nha Trang=`--green`, Đà Lạt=`--accent`, chi nhánh mới chưa định nghĩa màu → xám `#9AA7BD`), theo bộ lọc khoảng thời gian + Chi nhánh. Có **legend** (`#trendLegend`) liệt kê chi nhánh đang xuất hiện trong dữ liệu đã lọc, và **vạch mờ** (`.month-start`) đánh dấu ranh giới giữa các tháng (bar đầu tiên của tháng mới, trừ bar đầu biểu đồ). **Trục X ghi tên tháng** (đổi 23/09/2026 từ ghi ngày cụ thể dd/mm/yyyy ở 3 mốc đầu/cuối/đỉnh — giờ ghi "Tháng M/yyyy" tại đúng cột đầu tiên của mỗi tháng, khớp vị trí vạch mờ `.month-start`).
