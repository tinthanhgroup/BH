...
- Cột **Tốc độ bán (5 tuần gần nhất)**: **tổng** số xe chốt cọc trong 5 tuần ISO gần nhất (không chia trung bình/tuần — theo yêu cầu người dùng), tính từ `allCoc` (field `ngayCoc`) lọc đúng `model`+`phienBan`, dùng chung hàm tuần `wKey()`/`isoWeek()` với biểu đồ "Chốt cọc 5 tuần gần nhất" (tab Bán hàng) để nhất quán cách tính tuần. Hàm `_last5WeekKeys()` luôn trả đủ 5 tuần kể cả tuần không có xe nào.
