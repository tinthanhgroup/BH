...
- Thứ tự cột số liệu (từ trái sang, theo yêu cầu người dùng 09/2026): Tồn → BO → LXX → Đặt hàng → Tổng → Nợ → Tổng − Nợ → Số lượng xe ký (30 ngày gần nhất).
- **Hàng đánh số cột** (thêm 09/2026, do bảng nhiều cột): 1 hàng `<tr>` phụ ngay trên hàng tiêu đề trong `<thead>`, đánh số 1→`numCols` (hiện =12: Loại xe + 3 chi nhánh + 8 cột số liệu). Thêm/bớt cột thì nhớ sửa `numCols` cho khớp, không tự đếm lại.
