...
| `TinThanh_FBAds_Sync.gs` | Đọc Sheet quảng cáo Facebook → `fbads.json` — chi tiết [docs/module-fbads.md](docs/module-fbads.md). ⚠️ **Khác mọi script sync khác trong repo**: Sheet nguồn chỉ chứa snapshot chi phí của **hôm nay** (bị 1 tool ngoài ghi đè mỗi lần chạy, không tự cộng dồn lịch sử) — script phải tự **GET fbads.json hiện có trên GitHub, upsert theo khoá `ngày+tên chiến dịch`, rồi mới PUT lại** để không mất lịch sử các ngày trước (khác hẳn kiểu "Sheet là nguồn đầy đủ, ghi đè thẳng" của các script kia) | `fbAdsSync`: 1 trigger `everyHours(1)` (24 lần/ngày, 24/7) | `fbads.json` |
