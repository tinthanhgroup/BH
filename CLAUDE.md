...
Các trang nhúng iframe (`chamcong/nhansu/nhacungcap/mkt/theo_doi_chinh_sach/quy_dinh_noi_bo/crm.html`) đều **lazy-load** — chỉ set `iframe.src` khi người dùng thực sự bấm vào tab/sub-tab đó lần đầu (xem `_doSwitchTab()`/`switchNsSub()` trong `index.html`), tránh tải dữ liệu thừa.
