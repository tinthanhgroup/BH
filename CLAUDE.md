...
- Tiêu đề `<h1>` đầu trang: cỡ chữ `24px`, `font-weight:800` — **nhưng trang nào nhúng trong 1 sub-tab (`.subtabs`/`.subtab` ở `index.html`, không phải tab cấp cao nhất) thì KHÔNG cần `<h1>`** (đổi 09/2026, theo yêu cầu người dùng, áp dụng đồng loạt cho cả 6 sub-tab: 4.1 `nhansu.html`, 4.2 `chamcong.html`, 4.3 `nhacungcap.html`, 4.4 `quy_dinh_noi_bo.html`, 5.1 `fbads.html`, 5.2 `mkt.html`) — vì nhãn sub-tab ở `index.html` (vd "5.1. Quảng cáo Facebook") đã thay vai trò tiêu đề, có `<h1>` riêng nữa là trùng lặp. Các phần tử khác trong header (dòng "Cập nhật lúc...", mô tả ngắn dạng `<p>`, nút hành động như "Xem tất cả bài đăng Facebook") **vẫn giữ nguyên**, chỉ bỏ đúng `<h1>`. Áp dụng cho module sub-tab mới thêm sau này luôn, không cần hỏi lại. Trang KHÔNG nhúng trong sub-tab (`theo_doi_chinh_sach.html` ở tab 6, `crm.html` ở tab 7 — 2 trang này nhúng thẳng trong tab cấp cao nhất, không phải sub-tab) vẫn giữ `<h1>` như cũ.
