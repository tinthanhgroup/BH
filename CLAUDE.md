...
|---|---|---|---|
| `index.html` | Trang chính, desktop. 7 tab: 1.Hàng hóa · 2.BC Bán hàng · 3.BC Dịch vụ · 4.HCNS (Nhân sự & Chấm công, 4 sub-tab đánh số trong cùng 1 tab) · 5.MKT Thương hiệu (2 sub-tab, thêm 09/2026) · 6.Chính sách HTV (mở tự do) · 7.CRM + tab Admin | Từng tab khoá riêng theo mật khẩu + nhóm quyền (xem bên dưới); riêng tab 4 khoá theo **từng sub-tab** (xem chi tiết ngay dưới bảng) | Tab Hàng hóa → [docs/module-hanghoa.md](docs/module-hanghoa.md) |
| `mobile.html` | Bản rút gọn cho di động, KCK_DATA/KCK_DATE trùng với index.html — **sửa 1 nơi thì nhớ sửa nơi kia**. Không có tab 4 (HCNS) lẫn tab 5 (MKT Thương hiệu/FB Ads) | — | — |
...
| `mkt.html` | Báo cáo tổng hợp MKT Thương hiệu (Nhật ký đăng bài Fanpage), nhúng iframe trong sub-tab "5.1. Nhật ký đăng bài" của tab 5 | Không tự có auth (như trên) | [docs/module-mkt.md](docs/module-mkt.md) |
| `fbads.html` | Dashboard Quảng cáo Facebook (chi phí, kết quả Messenger, cảnh báo chiến dịch kém hiệu quả), nhúng iframe trong sub-tab "5.2. Quảng cáo Facebook" của tab 5 (thêm 09/2026) | Không tự có auth (như trên) — dùng chung quyền với sub 5.1, khoá ở cấp tab 5 | [docs/module-fbads.md](docs/module-fbads.md) |
...
**Tab 5 "MKT Thương hiệu" có 2 sub-tab** (thêm 09/2026, cơ chế tương tự tab 4 nhưng đơn giản hơn — **không khoá riêng từng sub**): "5.1. Nhật ký đăng bài" (`mkt.html`, mặc định mở khi vào tab) và "5.2. Quảng cáo Facebook" (`fbads.html`), chuyển qua hàm `switchMktSub('mkt'|'fbads')`, mỗi sub lazy-load iframe riêng khi mở lần đầu. Khác tab 4: cả 2 sub dùng chung đúng 1 quyền xem tab `mkt` — `switchTab('mkt')` vẫn khoá ở **cấp tab** như cũ (không đẩy việc kiểm tra quyền xuống từng sub như tab 4), nên `switchMktSub()` không cần tham số `silent`/kiểm tra quyền riêng.
