...
| `TinThanh_NhaCungCap_Sync.gs` | Đọc Sheet "Đầu mối CSVC" (mỗi tab = 1 chi nhánh, người dùng tự cập nhật tay) → `nhacungcap.json` — chi tiết [docs/module-nhacungcap.md](docs/module-nhacungcap.md) | Không bắt buộc trigger tự động (dữ liệu gần như không đổi) — chạy tay `nhaCungCapSync()` khi cập nhật Sheet, hoặc tự cài 1 trigger hàng ngày qua trang Triggers nếu muốn | `nhacungcap.json` |
